...
const login = function ($scope, $sce, httpManager, loadingManager) {
  ...
  function resetPasswordCallback(response) {
    ...
    if (response.data.status == 'failure') $scope.resetPasswordMessage = response.data.message;
  }
  $scope.login = () => {
    loadingManager.show();
    httpManager.postRequester('wp-admin/admin-ajax.php', {
  ...
  $scope.resetPassword = () => {
    ...
    httpManager.postRequester('wp-admin/admin-ajax.php', {
      ...
      email: $scope.email
    }, true, resetPasswordCallback, resetPasswordCallback);
  };
};
